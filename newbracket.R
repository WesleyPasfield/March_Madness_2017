...
#Change args 1 and three to the files you are using.
############################################
setwd("~/Documents/Kaggle/MarchMadness")
############################################
...
#Your file name may be different here
###############################################
...
names(seeds) <- c('season', 'seed', 'team', 'region')
seeds <- filter(seeds, season == 2016)
submission$season<-substr(submission[,1],1,4)
submission$team<-as.numeric(substr(submission[,1],6,9))
submission<-merge(submission,seeds, by = 'team')
submission <- submission[,c(4, 1, 2, 3, 6, 7)]
...
if (!is.na(solfn)) {    
    ...
    submission$pred1[!is.na(submission$result)] <- submission$result[!is.na(submission$result)]
}
